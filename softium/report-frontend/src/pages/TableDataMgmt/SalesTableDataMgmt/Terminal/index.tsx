...
import { Select, Space, Spin, TreeSelect } from 'antd';
...
import JurisdictionDetails from '@/pages/TableDataMgmt/SalesTableDataMgmt/Detail/JurisdictionDetails';
...
const Terminal: React.FC<TerminalProps> = ({ location }) => {
  ...
  const columns: ProColumns<TerminalList>[] = [
    {
      title: '销售年月',
      key: 'periodId',
      hideInTable: true,
    ...
    {
      title: '下游机构主类型',
      key: 'attachedToInstMainCategory',
      hideInTable: true,
      renderFormItem: (_, { type, defaultRender, ...rest }, form) => {
        return (
          <Select
            placeholder={'请选择'}
            onSelect={t => {
              form.resetFields(['attachedToInstCategory']);
            }}
            // mode={'multiple'}
            maxTagCount={2}
            allowClear={true}
    ...
    {
      title: '下游机构类型',
      key: 'attachedToInstCategory',
      hideInTable: true,
      formItemProps: {
        dependencies: ['attachedToInstMainCategory'],
      },
      renderFormItem: (_, { type, defaultRender, ...rest }, form) => {
        let mainCategory = form.getFieldValue('attachedToInstMainCategory');
        let optionsData = [];
        switch (mainCategory) {
          case '其他':
    ...
    {
      title: '下游经销商级别',
      key: 'attachedToInstDistributorLevelCode',
      hideInTable: true,
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          // search:false,
          render: (_, entity: TerminalList) => {
            return <JurisdictionDetails item={entity} key={entity.id} />;
          },
        },
        {
          title: '辖区负责人',
        ...
        },
        {
          ...
          width: 120,
          // render: (_, entity: TerminalList) => (
          //   <span>{entity?.terySysTargetType === 'Target' ? '是' : '否'}</span>
          // ),
        },
        {
          title: '辖区类型',
        ...
        // {
        //   title: '+1级辖区负责人',
        //   dataIndex: 'teryNodeParentOwnerName',
        //   search: false,
        // },
        // {
        //   title: '+1级辖区层级',
        //   dataIndex: 'teryNodeParentLevelName',
        //   search: false,
        // },
        // {
        //   title: '+2级辖区编码',
        //   dataIndex: 'toTeryParent2Code',
        //   search: false,
        // },
        // {
        //   title: '+2级辖区名称',
        //   dataIndex: 'toTeryParent2Name',
        //   search: false,
        // },
        // {
        //   title: '+2级辖区负责人',
        //   dataIndex: 'toTeryParent2OwnerName',
        //   search: false,
        // },
        // {
        //   title: '+3级辖区编码',
        //   dataIndex: 'toTeryParent3Code',
        //   search: false,
        // },
        // {
        //   title: '+3级辖区名称',
        //   dataIndex: 'toTeryParent3Name',
        //   search: false,
        // },
        // {
        //   title: '+3级辖区负责人',
        //   dataIndex: 'toTeryParent3OwnerName',
        //   search: false,
        // },
        {
          title: '辖区是否拆分',
    ...
    },
    {
      ...
      fixed: 'right',
      width: 130,
      render: (_: ReactNode, entity: TerminalList, index: number) => {
        return [
          <a
            key={entity.id}
            onClick={() => {
  ...
  if (windowTimeOption && windowTimeOption.length > 0) {
    return (
      <>
        <Table<TerminalList, TerminalListQuery>
          code={'SalesTable.Data.Mgmt.Terminal'}
          bordered={true}
          sticky={true}
          ...
          headerTitle={
            <Space>
              <DownloadData query={query} type={'terminal'} />
              <NewTemplate type="add" item={{}} />
              <TemplateList modelRef={listTableRef} />
            </Space>
          }
          beforeSearchSubmit={(params: TerminalListQuery) => {
            let userInfo = storage.get('userInfo');
            params.tenantId = userInfo?.tenantId;
            params.prodAffiliation = (params?.prodAffiliation || 0) * 1;
            params.toInstAffiliation = (params?.toInstAffiliation || 0) * 1;
            params.attachedToInstIsTerminal = params?.attachedToInstIsTerminal
              ? params?.attachedToInstIsTerminal * 1
              : undefined;
            setQuery(params);
            return params;
          }}
          ...
          scroll={{ x: 7000 }}
          columns={columns}
          request={async (params: TerminalListQuery) => {
