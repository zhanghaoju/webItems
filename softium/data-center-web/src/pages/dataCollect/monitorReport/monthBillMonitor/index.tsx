...
import SupplementMonthMonitor from './supplementModal';
...
interface GithubIssueItem {
  ...
  queryDate?: string;
  arriveStatusForSM?: number;
  arriveStatusForPM?: number;
  arriveStatusForIM?: number;
  arriveStatusForDM?: number;
  disabled?: number;
  IM_isArrive: number;
}
  ...
  const columns: ProColumns<GithubIssueItem>[] = [
    ...
    },
    {
      ...
      children: [
        {
          title: '是否到达',
          dataIndex: 'IM_isArrive',
          key: 'IM_isArrive',
          width: 100,
          render: (text, record, _, action) => [
            record.IM_isArrive === 0 ? (
              <React.Fragment>
                <span>
                  {transformText(
                    'whetherNot',
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setInstitutionCode(record.institutionCode);
  ...
  const getFileDashboardList = async (periodId: any, queryDate: any) => {
    ...
    try {
      ...
      setFileArrivalInfo({
        ...
        arrivedSM: res.data.arrivedSM, //已到达销售
        arrivedPM: res.data.arrivedPM, //已到达采购
        arrivedIM: res.data.arrivedIM, //已到达库存
        arrivedDM: res.data.arrivedDM, //已到达发货
    ...
    <div id="billMonitor">
      ...
      <div className="fileArrivalSurvey">
        <Spin spinning={loading}>
          <Card title="文件到达概况" bordered={false}>
            {ifFileEmpty ? (
              <Empty
          ...
          <Card title="经销商明细" bordered={false}>
            <div className="search-result-list">
              <Table<GithubIssueItem>
                code="monitorReport-monthBillMonitor"
                columns={columns}
                bordered
                pagination={{
                  defaultPageSize: 10,
                ...
                headerTitle={
                  <Space>
                    <Authorized code={'monthBillMonitor-export'}>
                      <Button
                        type="primary"
                        onClick={() => billMonitorExport()}
                      >
                        导出
                      </Button>
                    </Authorized>
      ...
      {supplementModalVisible && (
        <SupplementMonthMonitor
          institutionCode={institutionCode}
          institutionName={institutionName}
          setSupplementModalVisible={setSupplementModalVisible}
          supplementModalVisible={supplementModalVisible}
          periodId={periodId}
        />
      )}
    </div>
