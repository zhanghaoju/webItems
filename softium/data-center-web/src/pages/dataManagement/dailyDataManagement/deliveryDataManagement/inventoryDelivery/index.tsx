...
const InventoryDelivery: React.FC<InventoryDeliveryProps> = props => {
    ...
    },
    {
      ...
      render: (text: any, record: any, index: any) => [
        <a
          ...
          onClick={() =>
            history.push(
              `/dataManagement/dailyDataManagement/deliveryDataManagement/inventoryDelivery/detail/${record.id}?sourceTabIndex=3&sourcePage=/dataManagement/dailyDataManagement/deliveryDataManagement`,
            )
          }
        >
          查看
  ...
  const inventoryDeliveryExport = (filter: any) => {
    ...
    getInventoryDeliveryDataExportRequest(searchParams).then((res: any) => {
      downloadFile(res);
      setTimeout(() => {
      ...
      <div>
        <div className="search-result-list">
          <Table<GithubIssueItem>
            code="dataManage-day-inventoryDelivery"
            saveSearchValue
            ...
            request={(params, sort, filter) => {
              return getDailyInventoryDeliveryList({
            ...
            headerTitle={
              <Space>
                <Authorized code={'dayDelInve-export'}>
                  <Button
                    type="primary"
                    key={'export'}
                    loading={exportLoading}
                    onClick={filter => inventoryDeliveryExport(filter)}
                  >
                    导出
                  </Button>
                </Authorized>
