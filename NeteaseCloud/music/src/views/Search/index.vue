...
// 2. 再定义methods里getListFn方法获取数据
// 3. 在点击事件的方法里调用getListFn方法拿到结果数据
// 4. 铺设页面(首页van-cell标签复制过来一份)
// 5. 把数据保存到data后，循环van-cell使用即可(切换歌手字段)
// 6. 互斥显示搜索结果和热搜关键词

// 目标：搜索框-显示结果
// 1. @input给val-search绑定事件
...
// 注意：加载更多数据后，一定要把loading改成false，保证下一次还能触发onload方法
...
export default {
  ...
  methods: {
    ...
    async inputFn() {
      this.page = 1; // 点击重新获取第一页数据
      // 目标：输入框改变-逻辑代码-慢点执行，降低触发的频率
      // 解决：防抖
      // 计时n秒最后执行一次，如果再次触发，重新计时
      ...
      this.timer = setTimeout(async () => {
        ...
        if (this.value.length === 0) {
          // 搜索关键字如果没有，就把搜索结果清空阻止网络请求发送(return)
          this.resultList = [];
          return
        }
        const res = await this.getListFn();
    ...
    async onLoad() {
      this.finished = false;// 点击新关键词可能有新的数据
